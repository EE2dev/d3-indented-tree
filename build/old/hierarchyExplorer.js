(function (global, factory) {
  ...
  */
  ...
  function computeLabelDimensions(sel) {
    var dimArray = [];
    var dimsPositive = new Map();
    var dimsNegative = new Map(); // for link labels on links going to the left
    var dims = void 0;
    sel.each(function (d) {
      var dimProperties = {};
  ...
  nodesAPI.computeNodeExtend = function (sel) {
    ...
    filteredSel.each(function (d) {
      ...
      d.nodeBar.connectorStart2 = getConnectorStart(d3.select(this), d, l);
      console.log(d.name + ": " + (d.nodeBar.connectorStart === d.nodeBar.connectorStart2) + " " + d.nodeBar.connectorStart + " = " + d.nodeBar.connectorStart2);
  ...
  var getConnectorStart = function getConnectorStart(sel, d, linkAPI) {
    ...
    if (!d.parent || d.y >= d.parent.y) {
      ...
      if (labelBBox.width !== 0) {
        ...
        cs = d.y + options$1.nodeLabelPadding + labelBBox.width;
      } else {
        // 1.2 nodeImage - no nodeLabel
        cs = d.y + imageBBox.width / 2;
      }
    } else {
      // 2 node to the left
      if (d.linkLabel && d.linkLabel.overlap) {
        // 2.1 linkLabel to the right of node
        cs = d.linkLabel.overlap;
      } else {
        cs = d.parent.y - d.y + linkAPI.getLinkStrength(d.parent, options$1) / 2 + 5;
      }
      return cs;
    }
  };
